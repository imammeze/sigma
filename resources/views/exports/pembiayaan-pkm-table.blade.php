...
<body>
    <div class="title">TABEL PENGABDIAN KEPADA MASYARAKAT (PKM) DTPR DAN PEMBIAYAAN</div>
    <table>
        <thead>
            <tr class="header">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-left">{{ $row->judul_pkm }}</td>
                <td>{{ $row->jumlah_mahasiswa }}</td>
                <td>{{ $row->jenis_hibah_pkm }}</td>
                <td>{{ strtoupper($row->sumber_dana) }}</td>
                <td>{{ $row->durasi }}</td>
